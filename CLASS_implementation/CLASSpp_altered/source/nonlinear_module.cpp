...
// DRMD analytical model setup
struct DrmdParams {
    double a1_0, a1_z, a2_0, a2_z, a3_0, a3_z, b1_0, b1_z, b2_0, b2_z, b3_0, b3_z;
};

// Parameter dictionaries translated to C++ structs
const DrmdParams alpha_params  = {-1.1068, 0.2427, -24.8801, -1.5559, -40.9410, -679.9237, 48.1013, 0.1129, 732.1212, -1.9686, 1067.6955, -984.2972};
const DrmdParams beta_params   = {12.2414, -80160.3352, 39.4951, 17.5683, -165374.3809, 14.2822, 6187.0727, 16.7335, 393189.6096, -657644.3145, 8290461.0047, 13.9385};
const DrmdParams baseline      = {16.0794, 0.1396, 1.8772, -3.5340, 54.5102, -2.9914, 20.9506, 0.1330, 6.4875, -0.3864, 69.4966, -3.0894};

double pade_model(double k, double z, const DrmdParams& p) {
    double u = 1.0 + z;
    double a1 = p.a1_0 * pow(u, p.a1_z);
    double a2 = p.a2_0 * pow(u, p.a2_z);
    double a3 = p.a3_0 * pow(u, p.a3_z);
    double b1 = p.b1_0 * pow(u, p.b1_z);
    double b2 = p.b2_0 * pow(u, p.b2_z);
    double b3 = p.b3_0 * pow(u, p.b3_z);

    double num = 1.0 + a1*k + a2*pow(k, 2) + a3*pow(k, 3);
    double den = 1.0 + b1*k + b2*pow(k, 2) + b3*pow(k, 3);

    return num / den;
}

double alpha_beta_pade_model(double k, double z, const DrmdParams& p) {
    double u = 1.0 + z;
    double a1 = p.a1_0 * pow(u, p.a1_z);
    double a2 = p.a2_0 * pow(u, p.a2_z);
    double a3 = p.a3_0 * pow(u, p.a3_z);
    ...
    double num = a1*k + a2*pow(k, 2) + a3*pow(k, 3);
    double den = 1.0 + b1*k + b2*pow(k, 2) + b3*pow(k, 3);

    return num / den;
}

double calculate_drmd_correction(double k, double z, double dNeff_shift, double fidm_shift) {
    double alpha_val = 0.0;
    double beta_val = 0.0;
    double C0 = 0.0;

    if (dNeff_shift != 0.0) alpha_val = alpha_beta_pade_model(k, z, alpha_params);
    if (fidm_shift != 0.0)  beta_val  = alpha_beta_pade_model(k, z, beta_params);
    C0 = pade_model(k, z, baseline);
    return C0 + alpha_val * dNeff_shift + beta_val * fidm_shift;
}
...
int NonlinearModule::nonlinear_init() {
  ...
  /** --> Then go through common preliminary steps to the HALOFIT and HMcode methods */
    ...
    // If DRMD is active, intercept and boot up a parallel LCDM cosmology which is then corrected using the analytical model. 
    if ((pba->has_idm_drmd == _TRUE_) || (pba->has_idr_drmd == _TRUE_) || 
        (pba->delta_Neff_drmd > 0.0) || (pba->f_idm_drmd > 0.0)) {

        // 1. Create a pristine, isolated FileContent for the LCDM baseline
        FileContent lcdm_fc;

        // Loop through the original inputs and copy everything except the DRMD flags
        input_module_->file_content_.for_each([&lcdm_fc](const std::string& name, const std::string& value, bool read) {
            if (name == "has_idm_drmd" || name == "has_idr_drmd" ||
                name == "delta_Neff_drmd" || name == "f_idm_drmd" ||
                name == "z_stop" || name == "G_over_aH_drmd_ini") {
                return; // Skip the DRMD parameters
            }
            lcdm_fc.set(name, value);
        });

        // 2. Turn off DRMD in the LCDM FileContent
        lcdm_fc.set("has_idm_drmd", "no");
        lcdm_fc.set("has_idr_drmd", "no");
        lcdm_fc.set("delta_Neff_drmd", "0.0");
        lcdm_fc.set("f_idm_drmd", "0.0");
        lcdm_fc.set("z_stop", "0.0");
        lcdm_fc.set("G_over_aH_drmd_ini", "0.0");

        // 3. Create the nested InputModule and Cosmology
        std::unique_ptr<InputModule> lcdm_input{new InputModule(lcdm_fc)};

        // Turn off verbose printing for the nested run
        lcdm_input->background_.background_verbose = 0;
        lcdm_input->thermodynamics_.thermodynamics_verbose = 0;
        lcdm_input->perturbations_.perturbations_verbose = 0;
        lcdm_input->primordial_.primordial_verbose = 0;
        lcdm_input->nonlinear_.nonlinear_verbose = 0;

        // 4. Boot up the parallel LCDM cosmology
        Cosmology lcdm_cosmo{std::move(lcdm_input)};
        NonlinearModulePtr lcdm_nl = lcdm_cosmo.GetNonlinearModule();
        auto lcdm_bg = lcdm_cosmo.GetBackgroundModule();

        // 5. Extract data and apply the analytical correction C(k, z)
        std::vector<double> pvecback(background_module_->bg_size_);
        int last_index = 0;

        // Copy the index indicating where non-linearities stop being computable
        index_tau_min_nl_ = lcdm_nl->index_tau_min_nl_;

        for (int index_tau = tau_size_ - 1; index_tau >= 0; index_tau--) {
            
            // Get physical redshift z for this exact DRMD step
            class_call(background_module_->background_at_tau(tau_[index_tau], pba->short_info, pba->inter_normal, &last_index, pvecback.data()),
                       background_module_->error_message_,
                       error_message_);
            double z_val = pba->a_today / pvecback[background_module_->index_bg_a_] - 1.0;

            for (int index_pk = 0; index_pk < pk_size_; index_pk++) {
                
                // Copy the non-linear scale k_nl
                k_nl_[index_pk][index_tau] = lcdm_nl->k_nl_[index_pk][index_tau];

                if (index_tau >= tau_size_ - ln_tau_size_) {
                    int index_tau_late = index_tau - (tau_size_ - ln_tau_size_);
                    
                    // Prepare temporary arrays for the LCDM P_NL and its spline derivatives
                    std::vector<double> out_pk_lcdm(lcdm_nl->k_size_);
                    std::vector<double> ddout_pk_lcdm(lcdm_nl->k_size_);
                    
                    // 1. Get LCDM P_NL on its own k-grid at exactly z_val
                    class_call(lcdm_nl->nonlinear_pk_at_z(logarithmic, pk_nonlinear, z_val, index_pk, out_pk_lcdm.data(), NULL),
                               lcdm_nl->error_message_, error_message_);
                               
                    // 2. Spline it along the LCDM k-grid
                    class_call(array_spline_table_columns(const_cast<double*>(lcdm_nl->ln_k_.data()),
                                                          lcdm_nl->k_size_, out_pk_lcdm.data(), 1, ddout_pk_lcdm.data(),
                                                          _SPLINE_NATURAL_, error_message_),
                               error_message_, error_message_);
                    
                    int last_k_idx = 0;

                    for (int index_k = 0; index_k < k_size_; index_k++) {
                        double my_dNeff = pba->delta_Neff_drmd;
                        double my_fidm = pba->f_idm_drmd;
                        double k_val = k_[index_k];

                        // Convert the DRMD parameters into the normalized shifts used by the analytical model
                        double dNeff_shift = (my_dNeff - 0.83) / 0.231;
                        double fidm_shift = (my_fidm - 0.0285) / 0.0076;

                        double correction_C = calculate_drmd_correction(k_val, z_val, dNeff_shift, fidm_shift);
                        correction_C = MAX(correction_C, 1e-15);

                        // 3. Evaluate the splined LCDM spectrum exactly at the DRMD k_val
                        double target_ln_k = ln_k_[index_k];
                        double ln_k_min = lcdm_nl->ln_k_[0];
                        double ln_k_max = lcdm_nl->ln_k_[lcdm_nl->k_size_ - 1];
                        
                        // Protect against interpolation out-of-bounds due to differing k_max endpoints
                        if (target_ln_k > ln_k_max) target_ln_k = ln_k_max;
                        if (target_ln_k < ln_k_min) target_ln_k = ln_k_min;

                        double ln_pk_nl_lcdm;
                        class_call(array_interpolate_spline(const_cast<double*>(lcdm_nl->ln_k_.data()),
                                                            lcdm_nl->k_size_, out_pk_lcdm.data(), ddout_pk_lcdm.data(),
                                                            1, target_ln_k, &last_k_idx, &ln_pk_nl_lcdm, 1, error_message_),
                                   error_message_, error_message_);

                        // 4. Apply the correction
                        ln_pk_nl_[index_pk][index_tau_late * k_size_ + index_k] = ln_pk_nl_lcdm + log(correction_C);

                        // 5. Update nl_corr_density safely so velocity functions don't break
                        double P_NL = exp(ln_pk_nl_[index_pk][index_tau_late * k_size_ + index_k]);
                        double P_L  = exp(ln_pk_l_[index_pk][index_tau_late * k_size_ + index_k]);
                        nl_corr_density_[index_pk][index_tau * k_size_ + index_k] = sqrt(P_NL / P_L);
                        }
                      } 
                      else {
                    // For early times where we don't apply the correction, just default the density ratio to 1
                    for (int index_k = 0; index_k < k_size_; index_k++) {
                        nl_corr_density_[index_pk][index_tau * k_size_ + index_k] = 1.0;
                    }
                }
            }
        }
